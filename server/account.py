...
<header class="top"><a class="logo" href="/">FINO<b>STAT</b></a><div class="r"><a href="/dashboard">TERMINAL</a><a href="/brief">BRIEF</a><a href="/finch">FINCH</a><a href="/auth/logout" id="logout">SIGN OUT</a></div></header>
...
<p>Payments are processed by Razorpay (UPI, cards, net banking, wallets). Finostat never sees your card or UPI details.</p>
...
plansEl.addEventListener('click',function(e){
  ...
  if(!b) return; var plan=b.getAttribute('data-plan'); b.disabled=true; msg.hidden=true;
  fetch('/api/pay/order',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json','X-Requested-With':'fetch'},body:JSON.stringify({plan:plan,period:period})})
  .then(function(x){ return x.json().then(function(d){ d._ok=x.ok; return d; }); }).then(function(o){
    if(!o._ok){ b.disabled=false; show('bad',o.error||'Could not create the order'); return; }
...
def render(user: dict, status: dict, history: list[dict]) -> bytes:
    cat = pm.catalogue()
    plan = status["plan"]
    ...
    rows = "".join(
        ...
        f"<td>₹{r['amount'] / 100:,.2f}</td><td>{'paid' if r['status'] == 'paid' else 'not completed'}</td><td>{_esc(r['payment_id'] or '—')}</td></tr>"
        for r in history)
    ...
    doc = (_PAGE.replace("__CSS__", _CSS).replace("__EMAIL__", _esc(user["email"])).replace("__PLAN__", label).replace("__PLAN_SUB__", _esc(sub))
           .replace("__UPGRADE_HEAD__", head).replace("__NOTE__", note).replace("__HISTORY__", hist).replace("__DATA__", data).replace("__JS__", _JS))
