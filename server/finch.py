...
function render(){
  ...
  Array.prototype.forEach.call(W,function(el){
    ...
    if(t==='spot'){
      ...
      if(!c||!c.rows){ el.innerHTML='<span class="loading">loading the live NIFTY chain…</span>'; return; }
      var i=c.rows.findIndex(function(r){ return r.atm; }); var rows=c.rows.slice(Math.max(0,i-3),i+4);
      el.innerHTML='<div class="scrollx"><table><thead><tr><th>CE LTP</th><th>CE IV</th><th>CE Δ</th><th>STRIKE</th><th>PE Δ</th><th>PE IV</th><th>PE LTP</th></tr></thead><tbody>'+rows.map(function(r){
        var ce=r.ce||{},pe=r.pe||{}; return '<tr'+(r.atm?' style="background:rgba(106,53,240,.25)"':'')+'><td style="color:var(--text)">'+(ce.ltp!=null?fmt(ce.ltp):'—')+'</td><td>'+(ce.iv!=null?ce.iv.toFixed(1)+'%':'—')+'</td><td>'+(ce.delta!=null?ce.delta.toFixed(2):'—')+'</td><td style="color:var(--gold)">'+r.strike+(r.atm?' <small style="color:var(--faint)">ATM</small>':'')+'</td><td>'+(pe.delta!=null?pe.delta.toFixed(2):'—')+'</td><td>'+(pe.iv!=null?pe.iv.toFixed(1)+'%':'—')+'</td><td style="color:var(--text)">'+(pe.ltp!=null?fmt(pe.ltp):'—')+'</td></tr>'; }).join('')+'</tbody></table></div><p>NIFTY spot '+fmt(c.spot)+' · expiry '+expiryStr(c.expiry)+' · '+(c.live?'live':'last traded')+'. Calls get cheaper as strikes rise; puts get dearer. The ATM row is where time value peaks.</p>';
    } else if(t==='straddle'){
