...
- /shop — Finostat merchandise (mugs, tees, hoodie, desk mat, mouse pad, mechanical keyboard, keycaps, journal, stickers, cap, poster, tumbler, sleeve, bottle, tote) with trader phrases; cart + Cashfree checkout, ships across India, /shop/order/<id> tracks an order.
...
INTENTS = [
    ...
    (("shop", "merch", "mug", "t-shirt", "tshirt", "tee", "hoodie", "keyboard", "desk mat", "mouse pad", "sticker", "tote", "poster", "buy goods"), "shop"),
...
def answer_local(question: str, ctx: dict) -> str:
    ...
    if intent == "vip":
        return ("VIP Indicator is the Indian markets buy·sell engine — market structure (BOS/CHoCH), order blocks, fair value gaps, liquidity sweeps and premium/discount blended with nine published systems into one non-repainting call on NIFTY, BANKNIFTY, FINNIFTY, SENSEX and every F&O stock, 5m to daily, with entry, stop and target. One-time ₹12,999 for lifetime access to the web app plus the TradingView Pine Script: /vip-indicator. Desk members already have it as the STRUCT panel in the terminal.")
    if intent == "struct":
    ...
    if intent == "shop":
        return "The Finostat shop at /shop has trader merch — Theta Gang and Max Pain mugs, Sell Premium and IV Crush tees, the Zero DTE hoodie, The Chain desk mat, a Gamma Flip mouse pad, the Desk 87 mechanical keyboard with BUY/SELL/HEDGE keys, a trade journal, Greeks stickers, caps, posters, tumblers and totes. Prices include GST, shipping across India, pay by UPI or card through Cashfree."
    if intent == "blog":
