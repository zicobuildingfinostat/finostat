...
print("\n=== reels ===")
rf = pathlib.Path(tempfile.mkdtemp()) / "reels.json"
rf.write_text(json.dumps(["https://www.instagram.com/reel/ABC123xyz/", "https://instagram.com/p/DEF456/?utm_source=ig", "https://example.com/reel/nope", "javascript:alert(1)"]))
r = V.load_reels(rf)
check("only instagram reel/post links, normalised", r == ["https://www.instagram.com/reel/ABC123xyz/", "https://www.instagram.com/reel/DEF456/"], r)
check("missing file -> []", V.load_reels(rf.parent / "none.json") == [])
check("shipped reels.json parses", isinstance(V.load_reels(), list))
rh = V.render_reels(r)
check("official embed blockquotes, no script inline", rh.count('class="instagram-media"') == 2 and "embed.js" not in rh and 'id="reels"' in rh)
check("section renders reels + both follow buttons", 'id="reels"' in V.render_section(vs, r) and V.INSTAGRAM_URL in V.render_section(vs, r) and V.CHANNEL_URL in V.render_section(vs, r))
check("embed.js loads lazily from the JS", "instagram.com/embed.js" in V.JS and "IntersectionObserver" in V.JS)
...
check("homepage carries the placeholders and nav link", "<!--VIDEOS-->" in idx and "/*VIDEOS_CSS*/" in idx and 'href="#watch"' in idx and '"sameAs":["https://www.youtube.com/@finostat","https://www.instagram.com/chase.alpha_/"]' in idx)
