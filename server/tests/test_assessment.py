...
print("\n=== quotes ===")
tok = res["token"]
check("submission carries a token + first/city", len(tok) > 15 and res["first"] == "Asha" and res["city"] == "Pune", res.get("first"))
_, e = st.add_quote(res["id"], "wrong-token", "nice", True); check("wrong token rejected", e == "Not found", e)
_, e = st.add_quote(res["id"], tok, "   ", True); check("empty quote rejected", e, e)
q, e = st.add_quote(res["id"], tok, "  Alerts by  email are\nexactly what I wanted  ", True)
check("quote stored, whitespace folded", e is None and q["quote"] == "Alerts by email are exactly what I wanted" and q["allow"] is True, (q, e))
_, e = st.add_quote(res["id"], tok, "again", False); check("second quote refused", e and "Already" in e, e)
qs = st.quotes(); check("quotes() lists it with the publish flag", len(qs) == 1 and qs[0]["quote_ok"] == 1 and qs[0]["name"] == "Asha Rao", qs)
check("migration is idempotent", (st._migrate(), True)[1])
