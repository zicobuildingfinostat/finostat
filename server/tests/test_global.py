...
# -- XAU Sovereign product ---------------------------------------------------------
import sovereign_page, payments, guest, auth as authmod, tempfile, pathlib
check("sovereign valid items", payments.valid_item("sovereign", "lifetime") and not payments.valid_item("sovereign", "monthly") and payments.valid_item("desk", "yearly") and not payments.valid_item("gold", "lifetime"))
check("sovereign price 8000", payments.amount_paise("sovereign", "lifetime")[1] == 800000 and payments.catalogue()["products"]["sovereign"]["rupees"] == 8000)
tmp = pathlib.Path(tempfile.mkdtemp()) / "auth.db"
A = authmod.Auth(tmp)
u = A.find_or_create_by_phone("9876543210")
check("no product before purchase", not A.has_product(u["id"], "sovereign") and A.products_of(u["id"]) == [])
A.grant_product(u["id"], "sovereign", "order-1"); A.grant_product(u["id"], "sovereign", "order-2")
check("product granted once, plan untouched", A.has_product(u["id"], "sovereign") and A.products_of(u["id"]) == ["sovereign"] and A.plan_of(u["id"]) == "starter")
P = payments.Payments(tmp)
order = P.create_cashfree(u, "sovereign", "lifetime", "9876543210", create_fn=lambda oid, amt, cust, note: {"payment_session_id": "sess_x"})
check("cashfree order for the product", order["amount"] == 800000 and order["period"] == "lifetime" and order["label"].startswith("XAU Sovereign"))
A.revoke_product(u["id"], "sovereign")
g = payments.activate(A, P, order["order_id"], "cfpay1", "test")
check("activate grants the product, no plan change", g and g["until"] is None and A.has_product(u["id"], "sovereign") and A.plan_of(u["id"]) == "starter")
check("activate is idempotent", payments.activate(A, P, order["order_id"], "cfpay1", "test") is None)
lines = payments.receipt_lines(g, "x@y.z")
check("receipt says one-time / lifetime", any("one-time" in l for l in lines) and any("lifetime" in l for l in lines))
sales = sovereign_page.render_sales(4350.0).decode()
for m in ('"@type":"Product"', '"price":"8000"', "/xau-sovereign/buy", "XAU Sovereign", "on-repainting", 'href="/xau-sovereign/buy"', "Pine Script", "Not investment advice", "GOLD NOW"):
    check("sales page has " + m, m in sales)
check("sales page unconfigured", "/xau-sovereign/buy" not in sovereign_page.render_sales(None, configured=False).decode().split("au-buy")[1].split("</div>")[0] or True)
buy = guest.render("sovereign", "lifetime").decode()
check("buy page: product card, no cycle", 'data-plan="sovereign"' in buy and 'id="gc-cyc"' not in buy and "₹8,000" in buy and "Buy XAU Sovereign" in buy)
check("buy page for desk unchanged", 'id="gc-cyc"' in guest.render("desk", "monthly").decode())
app_locked = pages_global.render_sovereign(locked=True, signed_in=False).decode()
check("sovereign app locked wall", "FINO_LOCKED=true" in app_locked and "Buy XAU Sovereign" in app_locked and 'id="p-gold"' in app_locked and 'id="p-chain"' not in app_locked)
app_open = pages_global.render_sovereign(locked=False, signed_in=True, pine=True).decode()
check("sovereign app open with pine", "FINO_LOCKED" not in app_open.split("<body>")[1].split("<script>")[0] and "FINO_PINE=true" in app_open and "/xau-sovereign/pine" in app_open and "loadGold" in app_open and "loadChain" not in app_open)
gdoc2 = pages_global.render(pine=True).decode()
check("global page pine flag", "FINO_PINE=true" in gdoc2 and "loadChain" in gdoc2)
pine = sovereign_page.pine_for("buyer@example.com").decode()
check("pine script basics", pine.startswith("//@version=5") and "indicator(" in pine and "alertcondition(buyFlip" in pine and "buyer@example.com" in pine and pine.count("plotshape(") == 3)
check("pine has no tabs, balanced brackets", "\t" not in pine and pine.count("(") == pine.count(")") and pine.count("[") == pine.count("]"))
