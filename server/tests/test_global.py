...
# -- Aurum Strike product ---------------------------------------------------------
import aurum_page, payments, guest, auth as authmod, tempfile, pathlib
check("aurum valid items", payments.valid_item("aurum", "lifetime") and not payments.valid_item("aurum", "monthly") and payments.valid_item("desk", "yearly") and not payments.valid_item("gold", "lifetime"))
check("aurum price 8000", payments.amount_paise("aurum", "lifetime")[1] == 800000 and payments.catalogue()["products"]["aurum"]["rupees"] == 8000)
tmp = pathlib.Path(tempfile.mkdtemp()) / "auth.db"
A = authmod.Auth(tmp)
u = A.find_or_create_by_phone("9876543210")
check("no product before purchase", not A.has_product(u["id"], "aurum") and A.products_of(u["id"]) == [])
A.grant_product(u["id"], "aurum", "order-1"); A.grant_product(u["id"], "aurum", "order-2")
check("product granted once, plan untouched", A.has_product(u["id"], "aurum") and A.products_of(u["id"]) == ["aurum"] and A.plan_of(u["id"]) == "starter")
P = payments.Payments(tmp)
order = P.create_cashfree(u, "aurum", "lifetime", "9876543210", create_fn=lambda oid, amt, cust, note: {"payment_session_id": "sess_x"})
check("cashfree order for the product", order["amount"] == 800000 and order["period"] == "lifetime" and order["label"].startswith("Aurum"))
A.revoke_product(u["id"], "aurum")
g = payments.activate(A, P, order["order_id"], "cfpay1", "test")
check("activate grants the product, no plan change", g and g["until"] is None and A.has_product(u["id"], "aurum") and A.plan_of(u["id"]) == "starter")
...
sales = aurum_page.render_sales(4350.0).decode()
for m in ('"@type":"Product"', '"price":"8000"', "/aurum/buy", "Aurum Strike", "on-repainting", 'href="/aurum/buy"', "Pine Script", "Not investment advice", "GOLD NOW"):
    check("sales page has " + m, m in sales)
check("sales page unconfigured", "/aurum/buy" not in aurum_page.render_sales(None, configured=False).decode().split("au-buy")[1].split("</div>")[0] or True)
buy = guest.render("aurum", "lifetime").decode()
check("buy page: product card, no cycle", 'data-plan="aurum"' in buy and 'id="gc-cyc"' not in buy and "₹8,000" in buy and "Buy Aurum Strike" in buy)
check("buy page for desk unchanged", 'id="gc-cyc"' in guest.render("desk", "monthly").decode())
app_locked = pages_global.render_aurum(locked=True, signed_in=False).decode()
check("aurum app locked wall", "FINO_LOCKED=true" in app_locked and "Buy Aurum Strike" in app_locked and 'id="p-gold"' in app_locked and 'id="p-chain"' not in app_locked)
app_open = pages_global.render_aurum(locked=False, signed_in=True, pine=True).decode()
check("aurum app open with pine", "FINO_LOCKED" not in app_open.split("<body>")[1].split("<script>")[0] and "FINO_PINE=true" in app_open and "/aurum/pine" in app_open and "loadGold" in app_open and "loadChain" not in app_open)
gdoc2 = pages_global.render(pine=True).decode()
check("global page pine flag", "FINO_PINE=true" in gdoc2 and "loadChain" in gdoc2)
pine = aurum_page.pine_for("buyer@example.com").decode()
