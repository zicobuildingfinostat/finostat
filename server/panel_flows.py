...
(function(){
  ...
  function bars(cv,rows,keyA,keyB,labelA,labelB,cumKey){ var g=ctx2d(cv), c=g.c, pad={l:56,r:10,t:16,b:20}; if(!rows.length){ c.fillStyle=C.faint; c.fillText('no data yet',pad.l,30); return; }
    var vals=[]; rows.forEach(function(r){ vals.push(r[keyA]||0); vals.push(r[keyB]||0); }); var m=Math.max.apply(null,vals.map(Math.abs))||1; var n=rows.length, bw=(g.w-pad.l-pad.r)/n; var mid=pad.t+(g.h-pad.t-pad.b)/2, half=(g.h-pad.t-pad.b)/2;
